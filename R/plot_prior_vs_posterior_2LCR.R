...
plot_prior_vs_posterior_2LCR <- function(
  ...
  a <- prev_ab[1]; b <- prev_ab[2]   # from your prior_input$prev
  rho_df_post <- tibble::tibble(
    value = as.numeric(Ypost_rho),
    quantity = "rho", test = "global", type = "Posterior"
  )
  p_rho <- ggplot() +
    # TRUE prior (flat) as dashed line
    stat_function(
      fun  = dbeta,
      args = list(shape1 = a, shape2 = b),
      aes(x = after_stat(x), color = "Prior"),
      linewidth = 0.8, linetype = 2
    ) +
    # Posterior as KDE — only colour, no fill
    geom_density(
      data = rho_df_post,
      aes(x = value, colour = "Posterior"),
      alpha = 0.35, adjust = 1.2
    ) +
    scale_colour_manual(values = c(Prior = "grey45", Posterior = "steelblue4")) +
    labs(x = expression(rho), y = "Density",
         title = sprintf("Prevalence (rho): TRUE Beta(%g,%g) prior vs Posterior", a, b)) +
    theme_minimal(base_size = 12) +
    guides(colour = guide_legend(title = NULL, override.aes = list(linetype = c(2, 1)))) +
