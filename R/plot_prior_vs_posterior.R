...
plot_prior_vs_posterior <- function(fit, nsim = 20000){
  ...
  # exact Beta prior + posterior KDE
  ...
  p_rho <- ggplot() +
    stat_function(
      ...
      mapping = aes(x = after_stat(x), colour = "Prior"),
      linewidth = 0.6, linetype = 2
    ) +
    geom_density(
      data = dat |> filter(quantity == "rho", type == "Posterior"),
      aes(x = value, fill = "Posterior", colour = "Posterior"),
    ...
    guides(
      fill = guide_legend(title = NULL, override.aes = list(alpha = 0.25)),
      colour = guide_legend(title = NULL, override.aes = list(linetype = c(2, 1)))
    ) +
